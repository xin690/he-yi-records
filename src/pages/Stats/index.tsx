...
import { PieChart, TrendingUp, BarChart, Calendar, Trophy } from 'lucide-react'

export function StatsPage() {
  const { isInitialized } = useInitStore()
  const { currentStats, availableMonths, selectedMonth, setSelectedMonth, transactions } = useTransactionStore()
  const [viewMode, setViewMode] = useState<'category' | 'trend' | 'monthly' | 'ranking'>('category')
  ...
  const getCategoryChartOption = () => {
    if (!currentStats?.categoryBreakdown || Object.keys(currentStats.categoryBreakdown).length === 0) {
      return null
    }
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="text-center p-3 bg-green-50 dark:bg-green-900 rounded-lg">
            ...
            <div className="text-lg font-bold text-green-600 dark:text-green-400">
              ¥{(currentStats?.totalIncome || 0).toLocaleString()}
            </div>
          </div>
          <div className="text-center p-3 bg-red-50 dark:bg-red-900 rounded-lg">
            ...
            <div className="text-lg font-bold text-red-600 dark:text-red-400">
              ¥{(currentStats?.totalExpense || 0).toLocaleString()}
            </div>
          </div>
          <div className="text-center p-3 bg-blue-50 dark:bg-blue-900 rounded-lg">
            <div className="text-sm text-blue-600 dark:text-blue-400">结余</div>
            <div className={`text-lg font-bold ${(currentStats?.balance || 0) >= 0 ? 'text-blue-600' : 'text-red-500'}`}>
              ¥{(currentStats?.balance || 0).toLocaleString()}
            </div>
          </div>
        </div>
      </div>
                  ...
                  <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <div className="flex items-center gap-3">
                      <span className={`w-6 h-6 rounded-full flex items-center justify-center text-sm ${
                        idx < 3 ? 'bg-yellow-500 text-white' : 'bg-gray-200 dark:bg-gray-700'
                      }`}>
                        {idx + 1}
                      </span>
                    ...
                    <div className="text-right">
                      <div className="font-semibold text-red-600">¥{item.amount.toFixed(2)}</div>
              ...
              <div className="space-y-2">
                {getTopMerchants().map((item, idx) => (
                  <div key={idx} className="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-800 rounded-lg">
                    <div className="flex items-center gap-3">
                      <span className={`w-6 h-6 rounded-full flex items-center justify-center text-sm ${
                        idx < 3 ? 'bg-blue-500 text-white' : 'bg-gray-200 dark:bg-gray-700'
                      }`}>
                        {idx + 1}
                      </span>
